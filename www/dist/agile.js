...
// --------------------config------------------------
agile.config(function($ionicConfigProvider) {
  $ionicConfigProvider.backButton.text(false);  // 设置返回按钮的文字
  $ionicConfigProvider.navBar.alignTitle('left');  // 设置导航条的标题的对其方式
});
    ...
    .then(function(res) {
      if (res) {
        userstoriesService.delete(userstoryId).then(function() {
          console.log('delete');
          $state.go('agile.userstories', {project: $scope.userstory.project}); // 传参projectId
        });
      } else {
        console.log('cancel');
      }
    ...
    .then(function(res) {
      if (res) {
        issuesService.delete(issueId).then(function() {
          console.log('delete');
          $state.go('agile.issues', {project: $scope.issue.project}); // 传参projectId
        });
      } else {
        console.log('cancel');
      }
...
agile.controller('TimelinesCtrl', function($scope, $stateParams, $state, timelinesService, ionicMaterialInk, ionicMaterialMotion) {
  ...
  $scope.goDetails = function(id) {
    if ($scope.timelines[id].event_type.split('.')[2] === 'delete') {
      ...
      $state.go('agile.wiki', {id: $scope.timelines[id].data.wikipage.id});
    } else if ($scope.timelines[id].event_type.split('.')[0] === 'issues') {
      $state.go('agile.issue', {id: $scope.timelines[id].data.issue.id});
    } else if ($scope.timelines[id].event_type.split('.')[0] === 'userstories') {
      $state.go('agile.userstory', {id: $scope.timelines[id].data.userstory.id});
    } else {
